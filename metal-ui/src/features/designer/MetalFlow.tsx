...
export const MetalFlow = (props: MetalFlowProps) => {
  ...
  const broadCastNodeStatus = useCallback(
    (nodes: string[], status: MetalNodeState) => {
  ...
  const setNodesStatus = useCallback(
    (nds: HotNode[]) => {
      const unAnalysisNds = nds
        .filter((nd) => nd[1] === MetalNodeState.UNANALYSIS)
        .map((nd) => nd[0]);
      const unAnalysised = broadCastNodeStatus(
        unAnalysisNds,
  ...
  const onConnect: OnConnect = useCallback(
    (connection: Connection) => {
      ...
      if (connection.target !== null) {
        setNodesStatus([
          [connection.target, MetalNodeState.UNANALYSIS, undefined],
        ]);
        modify(true);
      }
  ...
  const updateNode = useCallback(
    (id: string, newMetal: Metal) => {
      ...
      const unAnalysed = new Set(
        broadCastNodeStatus([id], MetalNodeState.UNANALYSIS).map(
          (nde) => nde[0]
        )
      );
  ...
  const deleteNode = useCallback(
    (id: string) => {
      ...
      const hotNodes: HotNode[] = flowInstance
        .getEdges()
  ...
  const addNode = useCallback(
    (nodeTmpl: MetalNodeProps) => {
      const allNodeIds = new Set(
        flowInstance.getNodes().map((node) => node.id)
      );
      ...
      const nodeProps = {
        ...
        inputs:
          nodeTmpl.type === MetalTypes.SOURCE ||
          nodeTmpl.type === MetalTypes.SETUP
            ? (id: string) => []
            : inputs,
        outputs:
          nodeTmpl.type === MetalTypes.SINK ||
          nodeTmpl.type === MetalTypes.SETUP
            ? (id: string) => []
            : outputs,
  ...
  const checkSpec = useCallback(
    (spec: Spec) => {
      const emptyMetals = spec.metals.filter(
        (metal) =>
          metal.props === undefined ||
          metal.props === null ||
          _.isEmpty(metal.props)
      );
      const inputsIllegalMetals = spec.metals
        .filter(
          (metal) =>
            !(
              metal.type === MetalTypes.SETUP ||
              metal.type === MetalTypes.SOURCE
            )
        )
        .map((metal) => ({
          metal: metal,
        ...
        .filter(
          ({ metal, inputs }) =>
            ((metal.type === MetalTypes.MAPPER ||
              metal.type === MetalTypes.SINK) &&
              inputs !== 1) ||
  ...
  const exportSubSpec = useCallback(
    (target: string, isContainTarget: boolean) => {
      const targetNd: Node<MetalNodeProps> | undefined = flowInstance.getNode(
        target
      );
      ...
      if (isContainTarget) {
        ...
        getIncomers(targetNd, nodes, edges).forEach((incomer) =>
          queue.push(incomer)
        );
      }
  ...
  return (
    ...
    >
      <Background
        variant={isPending ? BackgroundVariant.Lines : BackgroundVariant.Dots}
      />
      <Controls
        showZoom={true}
        showFitView={true}
        ...
        position={"top-right"}
      >
        <ControlButton onClick={onLayout} title={"Layout"}>
          <VscTypeHierarchy />
        </ControlButton>
      </Controls>
      <MiniMap></MiniMap>
