...
export function MetalPkgView(props: MetalPkgProps) {
  ...
  const className =
    classSubs.length > 0 ? classSubs[classSubs.length - 1] : "?";
...
export function MetalExplorer(props: MetalExplorerProps) {
  ...
  const [pkgFilter, setPkgFilter] = useState<Set<MetalTypes>>(
    new Set<MetalTypes>()
  );
  ...
  const pkgs =
    result === null
      ...
      : result.filter(
          (pkg) =>
            _.find(restrictPkgs, (restrictPkg) => restrictPkg === pkg.pkg) !==
            undefined
        );
