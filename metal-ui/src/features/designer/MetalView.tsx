...
export function MetalNodeStateTip(props: MetalNodeStateTipProps) {
  ...
  return (
    <Paper
      ...
      variant="outlined"
    >
      <IconButton color={color}>{metalNodeStateView(status)}</IconButton>
    </Paper>
  );
}
...
export function onConnectValid(
  ...
  const graphTopology: GraphTopology = new GraphTopology(
    nodesOnGraph,
    edgesOnGraph
  );
  let isValid = true;
  const inputs: Node<MetalNodeProps>[] = nodes.filter(
  ...
  const outputs: Node<MetalNodeProps>[] = nodes.filter(
...
export function MetalNode(props: NodeProps<MetalNodeProps>) {
  ...
  const badgeContent = useMemo(() => <MetalNodeStateTip status={status} />, [
    status,
  ]);
  ...
  const isInputReady = useCallback(() => {
    ...
    if (
      (type === MetalTypes.MAPPER || type === MetalTypes.SINK) &&
      inputNds === 1
    ) {
      return true;
    }
...
function useNodeAnalysis(
  token: string | null,
  id: string | undefined,
  scope: () => string[]
): [(spec: Spec, subSpec: Spec) => void, State] {
  ...
  const [run, status] = useDesignerAsync<AnalysisResponse>({
    ...
    onPending: () => {
      setFlowPending(true);
      setHotNodes(scope().map((nd) => [nd, MetalNodeState.PENDING, undefined]));
    },
    onError: (reason) => {
      setFlowPending(false);
      modify(true);
...
function NodeAnalysis(props: NodeAnalysisProps) {
  ...
  const [analysis, analysisStatus] = useNodeAnalysis(
    token,
    getProjectId(),
    scope
  );
  ...
  const onClick = () => {
    ...
    if (action) {
      ...
      if (
        checkResult.emptyMetals.length !== 0 ||
        checkResult.inputsIllegalMetals.length !== 0
      ) {
        const emptyMsg = `Property undefined: [${checkResult.emptyMetals.join(
          ","
        )}]`;
        const illegalInputsMsg = `Illegal inputs: [${checkResult.inputsIllegalMetals
          .map((m) => m.metal)
          .join(",")}]`;
