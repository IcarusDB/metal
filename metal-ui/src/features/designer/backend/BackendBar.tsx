...
function useBackendDeploy(
  ...
  const deploy = () => {
    ...
    if (deployId !== undefined && deployId !== null) {
      run(
        deployBackendOfId(token, deployId)
          .then(() => {
...
function useBackendUndeploy(
  ...
  const undeploy = () => {
    ...
    if (deployId !== undefined && deployId !== null) {
      run(
        undeployBackendOfId(token, deployId)
          .then(() => {
            return getBackendStatus(token, deployId);
          })
          .then((status) => {
...
function BackendControl(props: BackendControlProps) {
  ...
  const isPending =
    deployStatus === State.pending || undeployStatus === State.pending;
  ...
  const deployTip = () => {
    switch (deployStatus) {
      ...
      case State.pending:
        return <RingLoader size="1em" loading={true} />;
      default:
        return platformType;
    }
  };
...
function statusInfo(status: BackendStatus) {
  switch (status.current) {
    case BackendState.CREATED:
      return `Create at ${moment(status.createdTime).format(
        "YYYY-MM-DD HH:mm:ss"
      )}`;
    case BackendState.DOWN:
    ...
    case BackendState.FAILURE:
      return `Fail at ${moment(status.failureTime).format(
        "YYYY-MM-DD HH:mm:ss"
      )}`;
  }
}
...
function useAnalysis(
  token: string | null,
  id: string
): [() => void, State, AnalysisResponse | null] {
  ...
  const [run, status, result] = useDesignerAsync<AnalysisResponse>({
    onSuccess: (result) => {
      ...
      modify(false);
      setHotNodes([...analysed, ...unAnalysed]);
    },
    onPending: () => {
      setFlowPending(true);
      setHotNodes(
        flowAction
          .allNodes()
          .map((nd) => [nd.id, MetalNodeState.PENDING, undefined])
      );
    },
    onError: (reason) => {
      setFlowPending(false);
      ...
      if (errorMsg) {
        ...
        if (metalIds) {
          ...
          setHotNodes(
            flowAction
              .allNodes()
              .map((nd) => [nd.id, MetalNodeState.ERROR, errorMsg])
          );
        }
      } else {
        setHotNodes(
          flowAction
            .allNodes()
            .map((nd) => [nd.id, MetalNodeState.ERROR, "Fail to analysis."])
        );
      }
    },
  });
      ...
      if (errorMsg) {
        ...
        setHotNodes(
          flowAction
            ...
            .map((nd) => [
              nd.id,
              MetalNodeState.ERROR,
              "Fail to Submit Execution.",
            ])
        );
      }
    },
    ...
    onError: (reason) => {
      ...
      if (errorMsg) {
        ...
        setHotNodes(
          flowAction
            .allNodes()
            .map((nd) => [nd.id, MetalNodeState.ERROR, "Fail to Execute."])
        );
      }
    },
  });
...
function SyncExecInfo(props: SyncExecInfoProps) {
  ...
  useEffect(() => {
    ...
    return onBackendStatusChange((status, prev) => {
      ...
      if (
        status.current === BackendState.UP &&
        status.current !== prev?.current
      ) {
        sync();
      }
    });
...
function ExecuteBar(props: ExecuteBarProps) {
  ...
  const syncExecInfo = useMemo(() => <SyncExecInfo token={token} id={id} />, [
    id,
    token,
  ]);
