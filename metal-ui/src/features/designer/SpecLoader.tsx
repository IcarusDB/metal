...
export function useSpecLoader(
  token: string | null
): [() => void, State, SpecFlow | null, any] {
  const [spec] = useSpec();
  ...
  const load = useCallback(() => {
    ...
    const task = getAllMetalPkgsOfClasses(token, classes).then((metalPkgs) => {
      const nodeTmpls: (MetalNodeProps | undefined)[] = spec.metals
        .filter((metal) => metal.type !== undefined)
        .map((metal) => {
          const pkg = _.find(
            metalPkgs,
            (pkg: MetalPkg) => pkg.class === metal.type
          );
          if (pkg === undefined) {
            return undefined;
          }
