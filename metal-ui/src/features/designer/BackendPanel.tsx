...
export const BackendPanel = forwardRef(
  (props: BackendPanelProps, ref: ForwardedRef<BackendPanelHandler>) => {
    ...
    const [
      fetchBackendStatus,
      fetchBackendStatusStatus,
      backendStatus,
      fetchBackendStatusError,
    ] = useAsync<BackendStatus>();
    const [
      deployBackend,
      deployBackendStatus,
      deployResult,
      deployBackendError,
    ] = useAsync<DeployResponse>();
    const [
      undeployBackend,
      undeployBackendStatus,
      undeployResult,
      undeployBackendError,
    ] = useAsync<UnDeployResponse>();

    const error = useCallback(() => {
      if (fetchBackendStatusError !== null) {
        const backendStatusError: AxiosError<ApiResponseEntity> = fetchBackendStatusError;
        const apiResponseError = ApiResponse.extractErrorMessage(
          backendStatusError
        );
    ...
    const isDeploySuccess = useCallback(() => {
      ...
      if (
        backendStatus.current === BackendState.DOWN ||
    ...
    return (
      ...
      >
        <Grid container spacing={1}>
          ...
          >
            <BackendStatusView
              status={backendStatus === null ? undefined : backendStatus}
            />
          </Grid>
