      ...
      >
        {error !== null && (
          <Alert severity="error">
            {"Fail to parse your input into json format."}
          </Alert>
        )}
        <Paper>
          <Stack
...
export function BackendArgsProfile(props: BackendArgsProfileProps) {
  ...
  const uiSchema = {
    items: {
      "ui:widget": "textarea",
      "ui:description":
        "Item should be '{string}'. And \" in {string} should be escape by \\.",
    },
  };
  ...
  const check: () => [boolean, string | undefined] = useCallback(() => {
    ...
    if (platform !== null) {
      if (platform[basic.platform] === undefined) {
        return [
          false,
          `The project platform[${basic.platform}] is not configured.`,
        ];
      }
    }
...
export class MutableProjectProfileHandler extends Mutable<ProjectProfileHandler>
...
function platformWithType(
  platformProfile: any | null,
  basicProfile: ProjectBasicProfileValue | null
) {
  ...
  const withType = `{"${basicProfile.platform}": ${JSON.stringify(
    platformProfile
  )}}`;
...
export const ProjectProfile = forwardRef(
  (props: ProjectProfileProps, ref: ForwardedRef<ProjectProfileHandler>) => {
    ...
    const [
      newBasicProfile,
      setBasicProfile,
    ] = useState<ProjectBasicProfileValue | null>(basicProfile);
    const [newPkgProfile, setPkgProfile] = useState<PkgProfileValue | null>(
      pkgProfile
    );
    const [newPlatformProfile, setPlatformProfile] = useState<any>(
      platformProfile
    );
    const [newBackendArgsProfile, setBackendArgsProfile] = useState<
      string[] | null
    >(backendArgs);
    ...
    const handleNextStep = () => {
      setActiveStep(
        activeStep + 1 > STEP_SIZE - 1 ? STEP_SIZE - 1 : activeStep + 1
      );
    };
...
export const ProjectProfileViewer = forwardRef(
  (
    props: ProjectProfileViewerProps,
    ref: ForwardedRef<ProjectProfileViewerHandler>
  ) => {
