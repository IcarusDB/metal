...
public class Draft {
    ...
    public static class Builder {
        ...
        public Builder add(Metal metal) throws NullPointerException, NoSuchElementException {
            ((MutableGraph<Metal>) inner.graph).addNode(Optional.of(metal).get());
            return this;
        }

        public Builder addEdge(Metal source, Metal target)
            ...
            if (source instanceof MSink && sourceOutDegree > 0) {
                throw new IllegalArgumentException(
                        MSink.class + " can not output anything to the others.");
            }
        ...
        public Draft build() throws IllegalArgumentException {
            ...
            for (MFusion mFusion : mFusions) {
                if (inner.graph.inDegree(mFusion) < 2) {
                    String msg =
                            String.format("MFusion{%s} must have at least two inputs!", mFusion);
                    throw new IllegalArgumentException(msg);
                }
            }
    ...
    public static class WithWaitFor {
        ...
        public WithWaitFor waitFor(Metal affectedMetal, MSink mSink)
            ...
            if (graph.predecessors(mSink).contains(affectedMetal)) {
                String msg =
                        String.format(
                                "AffectedMetal %s can\'t be predecessor of MSink %s.",
                                affectedMetal, mSink);
                throw new IllegalArgumentException();
            }

            Set<MSink> affectedSinks =
                    graph.successors(affectedMetal).stream()
                            .filter(
                                    (Metal m) -> {
    ...
    public String toString() {
        ...
        String graph =
                StreamSupport.stream(
                                Traverser.forGraph(this.getGraph())
