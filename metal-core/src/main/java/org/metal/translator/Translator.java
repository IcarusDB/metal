...
public class Translator<D, S> {
  ...
  public void translate(Draft draft) throws MetalTranslateException {
    ...
    for (Metal metal : dependencyTrace) {
      try {
        ...
        throw new MetalTranslateException(
            "Illegal State happened, the Translator Context will not change.", e, metal.id());
      }
    }
    ...
    List<Metal> unStagingDependencyTrace = StreamSupport.stream(dependencyTrace.spliterator(),
            false)
        .filter(metal -> {
