...
public class MetalRepo extends AbstractVerticle {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MetalRepo.class);
                        ...
                        (JsonObject conf) -> {
                            ...
                            if (metalRepoConf == null) {
                                return Future.failedFuture(
                                        String.format(
                                                "%s is not configured in %s.",
                                                METAL_REPO_CONF, CONF_METAL_SERVER_PATH));
                            }

                            JsonObject metalRepoServiceConf =
                                    metalRepoConf.getJsonObject(METAL_REPO_SERVICE_CONF);
                            ...
                            String address =
                                    metalRepoServiceConf.getString(METAL_REPO_SERVICE_ADDRESS_CONF);
                            ...
                            provider =
                                    IMetalRepoService.createProvider(
                                            getVertx(), mongo, metalRepoServiceConf);
    ...
    public static class RestApi {
        ...
        private RestApi(Vertx vertx, String provider) {
            metalRepoService =
                    IMetalRepoService.create(vertx, new JsonObject().put("address", provider));
        }
        ...
        public void getAllOfUserScope(RoutingContext ctx) {
            ...
            metalRepoService
                    .getAllOfUserScope(userId, scope)
                    .onSuccess(
                            metals -> {
                                JsonObject resp = new JsonObject();
                                resp.put("status", "OK");
        ...
        public void getAllOfPkg(RoutingContext ctx) {
            ...
            if (OnFailure.doTry(
                    ...
                    "Fail to found groupId in request.",
                    400)) {
                return;
            }

            metalRepoService
                    .getAllOfPkg(userId, groupId, artifactId, version)
                    .onFailure(
                            error -> {
                                JsonObject resp = new JsonObject();
                    ...
                    .onSuccess(
                            metals -> {
                                JsonObject resp = new JsonObject();
                                resp.put("status", "OK").put("data", metals);
                                SendJson.send(ctx, resp, 200);
                            });
        }
        ...
        public void getAllOfType(RoutingContext ctx) {
            ...
            if (OnFailure.doTry(
                    ...
                    "Fail to found metalType in request.",
                    400)) {
                return;
            }

            metalRepoService
                    .getAllOfType(userId, metalType)
                    .onFailure(
                            error -> {
                                JsonObject resp = new JsonObject();
                                resp.put("status", "FAIL").put("msg", error.getLocalizedMessage());
        ...
        public void getOfClass(RoutingContext ctx) {
            ...
            if (OnFailure.doTry(
                    ...
                    "The class field in request is invalid",
                    400)) {
                return;
            }
        ...
        public void getAllOfClasses(RoutingContext ctx) {
            ...
            try {
                ...
                if (OnFailure.doTry(
                        ...
                        "The classes field in request is invalid",
                        400)) {
                    return;
                }
            }
        ...
        public void addFromManifest(RoutingContext ctx) {
            ...
            metalRepoService
                    .addFromManifest(userId, scope, manifest)
                    .onSuccess(
                            ret -> {
                                JsonObject resp = new JsonObject();
                                resp.put("status", "OK");
