                ...
                .compose(
                        (JsonObject conf) -> {
                            ...
                            JsonObject execServiceConf =
                                    backendReportConf.getJsonObject(EXEC_SERVICE_CONF);
                            JsonObject projectServiceConf =
                                    backendReportConf.getJsonObject(PROJECT_SERVICE_CONF);
                            JsonObject backendReportServiceConf =
                                    backendReportConf.getJsonObject(BACKEND_REPORT_SERVICE_CONF);
                            String backendReportServiceAddress =
                                    backendReportServiceConf.getString(
                            ...
                            if (projectServiceConf == null) {
                                return Future.failedFuture(
                                        String.format(
                                                "%s is not configured in %s.",
                                                PROJECT_SERVICE_CONF, CONF_METAL_SERVER_PATH));
                            }
                            if (backendReportServiceConf == null) {
                                return Future.failedFuture(
                                        String.format(
                                                "%s is not configured in %s.",
                                                BACKEND_REPORT_CONF, CONF_METAL_SERVER_PATH));
                            }
                            if (backendReportServiceAddress == null
                                    || backendReportServiceAddress.isBlank()) {
                                return Future.failedFuture(
                                        String.format(
                                                "%s is not configured in %s.",
                                                BACKEND_REPORT_SERVICE_ADDRESS_CONF,
