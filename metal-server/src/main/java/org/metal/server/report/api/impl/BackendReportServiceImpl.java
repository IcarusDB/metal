...
public class BackendReportServiceImpl implements BackendReportService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(BackendReportServiceImpl.class);

    private ExecService execService;
    private IProjectService projectService;

    public BackendReportServiceImpl(ExecService execService, IProjectService projectService) {
                ...
                .compose(
                        (JsonObject lastStatus) -> {
                            ...
                            if (!lastState.equals(ExecState.CREATE)) {
                                String msg =
                                        String.format(
                                                "The status of exec can\'t switch from %s to %s.",
    ...
    private static boolean checkBackendStatus(JsonObject report, BackendState expect)
        ...
        if (!report.containsKey("status")) {
            throw new IllegalArgumentException(
                    String.format("status is lost in %s.", report.toString()));
        }
        ...
        if (!expect.equals(backendState)) {
            throw new IllegalArgumentException(
                    String.format(
                            "The parameter %s is not in '%s' status.",
                            report.toString(), expect.toString()));
        }
        return true;
    }
    ...
    private static boolean checkLegalEpoch(int lastEpoch, int epoch, String deployId)
            throws IllegalArgumentException {
        if (lastEpoch == epoch) {
            return true;
        } else {
            String msg =
                    String.format(
                            ...
                            BackendReportError.EPOCH_ILLEGAL.toString(),
                            lastEpoch,
                            epoch,
                            deployId,
                            epoch);
        ...
        return execService
                ...
                .compose(
                        (JsonObject lastStatus) -> {
                            ...
                            ExecState lastState = ExecState.valueOf(lastStatus.getString("status"));
                            if (lastState.equals(ExecState.FAILURE)
                                    || lastState.equals(ExecState.FINISH)) {
                                String msg =
                                        String.format(
                                                "The status of exec is %s and terminated.",
                                                lastState.toString());
                                return Future.failedFuture(msg);
                            }
    ...
    public Future<Void> reportBackendUp(JsonObject up) {
        ...
        return projectService
                .getBackendStatusOfDeployId(deployId)
                .compose(
                        (JsonObject lastStatus) -> {
                            try {
                                ...
                                return projectService
                                        .updateBackendStatusOnUpWith(deployId, epoch, current)
                                        .compose(
                                                ret -> {
                                                    LOGGER.info(
                                                            String.format(
                ...
                .compose(
                        (JsonObject lastStatus) -> {
                            try {
                                ...
                                maybeMarkedFailure(lastStatus);
                                return projectService
                                        .updateBackendStatusOnDownWith(deployId, epoch, current)
                                        .compose(
                                                ret -> {
                                                    LOGGER.info(
                                                            String.format(
                                                                    "Backend[%s-%d] has down.",
                                                                    deployId, epoch));
                                                    return Future.succeededFuture();
                                                });
                            } catch (Exception e) {
                            ...
                            try {
                                ...
                                return projectService
                                        .updateBackendStatusOnFailureWith(
                                                deployId, epoch, current, failureMsg)
                                        .compose(
                                                ret -> {
                                                    LOGGER.info(
                                                            String.format(
                                                                    "Backend[%s-%d] has failure: %s.",
    ...
    private static void maybeMarkedDown(JsonObject lastStatus) throws Exception {
        ...
        if (lastState.equals(BackendState.DOWN)) {
            String msg =
                    String.format(
                            "[%s] The status of exec has been marked %s and terminated.",
