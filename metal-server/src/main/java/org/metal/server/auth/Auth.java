...
public class Auth extends AbstractVerticle {
    ...
    private Auth(MongoClient client) {
        ...
        authorizationProvider =
                MongoAuthorization.create("authorization", mongo, new MongoAuthorizationOptions());
    ...
    public void updateUserName(RoutingContext ctx) {
        ...
        if (OnFailure.doTry(
                ...
                "Fail to found legal newName in request.",
                400)) {
            return;
        }

        JsonObject matcher = new JsonObject().put(UserDB.FIELD_ID, userId);
        JsonObject update =
                new JsonObject().put("$set", new JsonObject().put(UserDB.FIELD_USER_NAME, newName));
    ...
    public void updateUserPassword(RoutingContext ctx) {
        ...
        if (OnFailure.doTry(
                ...
                "Fail to found legal oldPassword in request.",
                400)) {
            return;
        }
        ...
        if (OnFailure.doTry(
                ...
                "Fail to found legal newPassword in request.",
                400)) {
            return;
        }
        ...
        JsonObject matcher =
                new JsonObject().put(UserDB.FIELD_ID, userId).put(UserDB.FIELD_PASSWORD, oldHash);
        JsonObject update =
                new JsonObject().put("$set", new JsonObject().put(UserDB.FIELD_PASSWORD, newHash));
        Future<JsonObject> result = UserDB.update(mongo, matcher, update);
        RestServiceEnd.end(ctx, result, LOGGER);
    }
        ...
        mongo.insert(
                ...
                .onSuccess(
                        userId -> {
                            ...
                            JsonObject resp = new JsonObject();
                            resp.put("status", "OK").put("userId", userId);
    ...
    public void createJWT(RoutingContext ctx) {
        ...
        if (user == null) {
            ...
            JsonObject resp = new JsonObject();
            resp.put("status", "FAIL").put("msg", "Fail to authenticate.");
        ...
        JsonObject resp = new JsonObject().put("status", "OK").put("jwt", jwt);
        SendJson.send(ctx, resp, 201);
    }
