...
public class SqlMFusion extends SparkMFusion<ISqlMFusionProps> {
    ...
    public Dataset<Row> fusion(SparkSession platform, Map<String, Dataset<Row>> datas)
        ...
        if (!this.props().tableAlias().keySet().equals(datas.keySet())) {
            String msg =
                    String.format(
                            ...
                            this.id(), datas.keySet(), this.props().tableAlias().keySet());
        ...
        if (tables.primary().size() <= 1) {
            String msg =
                    String.format(
                            "%s should access more than one primary table.", this.props().sql());
            throw new MetalTranslateException(msg);
        }
        ...
        if (isTablesOutRange(
                tables.primary(), new HashSet<String>(this.props().tableAlias().values()))) {
            String msg =
                    String.format(
                            "%s accessed table should be same with configured Table Alias{%s}.",
                            this.props().sql(), this.props().tableAlias().values());
            throw new MetalTranslateException(msg);
        }
